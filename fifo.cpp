...
queue<int> fifo_queue; // queue to store the page frames in main memory in FIFO order
                    ...
                    if(page_frames_mm_used < page_frames_mm)
                    {
                        for(int k = 0 ; k < page_frames_mm ;k++)
                        {
                            if(page_occupied_mm[k] == 0 || page_occupied_mm.find(k) == page_occupied_mm.end()) // page frame is not occupied
                            {
                                ...
                                fifo_queue.push(k) ; // pushing the page frame in the fifo queue
                                break;
                            }
                        }
                    } else {
    ...
    if(page_frames_mm_used < page_frames_mm)
    {
        for(int i = 0 ; i<page_frames_mm ;i++)
        {
            if(page_occupied_mm[i] == 0 || page_occupied_mm.find(i) == page_occupied_mm.end()) // page frame is not occupied
            {
                ...
                fifo_queue.push(i); // pushing the page frame in the fifo queue
                page_frames_vm_used--; // decreasing the number of pages used in swap space
                break;
            }
        }
    }    else {
        // if there is no free page frame in main memory then i need to swap a page frame in main memory using FIFO policy
        int frame_num_to_swap = fifo_queue.front(); // page frame in main memory which is to be swapped
        fifo_queue.pop();
     ...
     //   cout << p1.page_table.size() << endl;
       ...
       // cout << "page_num_to_swap" << page_num_to_swap << endl;
        ...
        fifo_queue.push(frame_num_to_swap); // pushing the page frame in the fifo queue
...
void kill(int pid)
{
    ...
    // removing the pageframes occupied by the process from the fifo queue
